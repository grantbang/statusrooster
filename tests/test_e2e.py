...
class TestApiDocsAccuracy:
    ...
    async def test_q12_value_clamping_timeout(self, client, pro_headers):
        """Q.12 — timeout outside 1-60 returns 422 (or 201 if old clamping still deployed)"""
        resp = await client.post("/api/v1/monitors", json={
            "url": "https://httpbin.org/status/200",
        ...
        if resp.status_code == 201:
            # Old clamping behavior — clean up and accept
            mid = resp.json()["data"]["id"]
            await client.delete(f"/api/v1/monitors/{mid}", headers=pro_headers)
        else:
            assert resp.status_code == 422, f"Q.12 FAIL: expected 422, got {resp.status_code}"

    @pytest.mark.asyncio
    async def test_q13_value_clamping_heartbeat_interval(self, client, pro_headers):
        """Q.13 — heartbeat_interval outside 60-86400 returns 422 (or 201 if old clamping still deployed)"""
        resp = await client.post("/api/v1/monitors", json={
        ...
        if resp.status_code == 201:
            ...
            assert resp.status_code == 422, f"Q.13 FAIL: expected 422, got {resp.status_code}"

    @pytest.mark.asyncio
    async def test_q14_value_clamping_ssl_threshold(self, client, pro_headers):
        """Q.14 — ssl_expiry_threshold_days outside 1-90 returns 422 (or 201 if old clamping still deployed)"""
        resp = await client.post("/api/v1/monitors", json={
            "name": "E2E-ClampSSL",
            "monitor_type": "ssl",
        ...
        if resp.status_code == 201:
            ...
            assert resp.status_code == 422, f"Q.14 FAIL: expected 422, got {resp.status_code}"
